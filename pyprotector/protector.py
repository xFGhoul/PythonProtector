...
class PythonProtector:
    def __init__(self, debug: bool, logs_path: Union[Path, str], webhook_url: str):
        ...
        LOGGING_CONFIG: Dict = {
        "handlers": [
            {"sink": self.logs_path, "format": "[{time:YYYY-MM-DD HH:mm:ss}] {module}::{function}({line}) - {message}", "enqueue": True, "rotation": "daily", "level": "INFO", "serialize": False, "backtrace": False, "catch": False},
        ],
        }
    ...
    def start(self):
        ...
        if self.debug:
            ...
            self.logger.info(f"Operating System: {platform.uname().system} {platform.uname().release} {platform.win32_edition()} ({platform.architecture(sys.executable)[0]})")
